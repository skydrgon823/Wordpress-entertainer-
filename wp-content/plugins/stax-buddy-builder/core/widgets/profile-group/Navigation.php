...
class Navigation extends \Buddy_Builder\Widgets\Base {
	...
	public function get_name() {
		return 'bpb-profile-group-navigation';
	}
	...
	protected function _register_controls() {
		...
		if ( isset( $appearance['group_front_page'] ) ) {
			$show_home_tab = $appearance['group_front_page'] ? 'yes' : 'no';
		}
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} nav ul li a:hover, {{WRAPPER}} #item-nav ul li a:hover' => 'border-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} nav ul li.selected a, {{WRAPPER}} #item-nav ul li.selected a',
			]
		);
