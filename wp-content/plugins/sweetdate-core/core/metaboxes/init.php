...
function cmb_oembed_ajax_results() {
	...
	if ( empty( $oembed_string ) ) {
		$return = '<p class="ui-state-error-text">' . esc_html__( 'Please Try Again', 'sweetdate' ) . '</p>';
		$found  = 'not found';
	} else {
		...
		if ( $check_embed && $check_embed != $fallback ) {
			// Embed data
			$return = '<div class="embed_status">' . $check_embed . '<a href="#" class="cmb_remove_file_button" rel="' . $_REQUEST['field_id'] . '">' .
			          esc_html__( 'Remove Embed', 'sweetdate' ) .
			          '</a></div>';
			// set our response id
			$found = 'found';
			...
			$return = '<p class="ui-state-error-text">' .
			          sprintf( esc_html__( 'No oEmbed Results Found for %s. View more info at', 'sweetdate' ), $fallback ) .
			          ' <a href="http://codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>.</p>';
			// set our response id
			$found = 'not found';
		}
	}
