...
if (! class_exists( 'SVQ_WXR_Parser' )) {
	/**
	 * WordPress Importer class for managing parsing of WXR files.
	 */
	class SVQ_WXR_Parser {
		function parse( $file ) {
			// Attempt to use proper XML parsers first
			if ( extension_loaded( 'simplexml' ) ) {
				$parser = new SVQ_WXR_Parser_SimpleXML;
			...
			if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
				...
				if ( 'SimpleXML_parse_error' == $result->get_error_code() ) {
					...
					echo $error[0] . ':' . $error[1] . ' ' . esc_html( $error[2] ); // WPCS: XSS OK.
				}
				echo '</pre>';
			...
			$parser = new SVQ_WXR_Parser_Regex;
...
if ( ! class_exists( 'SVQ_WXR_Parser_SimpleXML' ) ) {
	/**
	 * WXR Parser that makes use of the SimpleXML PHP extension.
	 */
	class SVQ_WXR_Parser_SimpleXML {
		function parse( $file ) {
...
if ( ! class_exists( 'SVQ_WXR_Parser_XML' ) ) {
	/**
	 * WXR Parser that makes use of the XML Parser PHP extension.
	 */
	class SVQ_WXR_Parser_XML {
		var $wp_tags = array(
			'wp:post_id',
			'wp:post_date',
			'wp:post_date_gmt',
...
if ( ! class_exists( 'SVQ_WXR_Parser_Regex' ) ) {
	/**
	 * WXR Parser that uses regular expressions. Fallback for installs without an XML parser.
	 */
	class SVQ_WXR_Parser_Regex {
		var $authors = array();
		var $posts = array();
