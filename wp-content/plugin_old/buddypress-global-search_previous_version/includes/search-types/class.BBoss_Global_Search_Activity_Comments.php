...
if (!class_exists('BBoss_Global_Search_Activity_Comment')):
	 ...
	 *
	 */
	class BBoss_Global_Search_Activity_Comment extends BBoss_Global_Search_Type {
		...
		public static function instance() {
			...
			if (null === $instance) {
				$instance = new BBoss_Global_Search_Activity_Comment();
			}
		...
		function sql( $search_term, $only_totalrow_count=false ){
            ...
            return apply_filters( 
                'BBoss_Global_Search_Activity_Comment_sql',
                $sql, 
                array( 
		...
		protected function generate_html( $template_type='' ){
			...
			if( bp_has_activities( array( 'display_comments' => 'stream', 'include'=>$post_ids, 'per_page'=>count($post_ids_arr) ) ) ){
				...
				while ( bp_activities() ){
					...
					$result = array(
						...
						'html'	=> buddyboss_global_search_buffer_template_part( 'loop/activity-comment', $template_type, false ),
					);
